...
def ParseYOLOV3Output(blob, resized_im_h, resized_im_w, original_im_h, original_im_w, threshold, objects):
    ...
    side_square = side * side # 26 * 26 / 13 * 13
    output_blob = blob.flatten() # flatten : 일렬
    
    for i in range(side_square): 
        row = int(i / side) # i / 26 
        col = int(i % side) # i % 26
        for n in range(num):
            obj_index = EntryIndex(side, coords, classes, n * side * side + i, coords) # 26,4,80,3*26*26+i,4
            box_index = EntryIndex(side, coords, classes, n * side * side + i, 0) # 26,4,80,3*26*26+i,0
